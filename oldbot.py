...
class MyClient(discord.Client):
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if message.author.id == self.user.id:
        ...
        elif ord(message.content[1]) > 48 and ord(message.content[1]) < 58:
            if 'd' not in message.content: return
            message_index = 1
            dice_size = ''
            dice_number = ''
            while message.content[message_index] != 'd':
                dice_number += message.content[message_index]
                message_index+=1
            message_index+=1
            if message_index >= len(message.content):
                message.channel.send("Are you missing the dice size?")
                return
            
            try:
                while (ord(message.content[message_index]) > 47 and ord(message.content[message_index]) < 58):
                    dice_size += message.content[message_index]
                    message_index+=1
            except:
                pass
            ...
            bonus = 0
            if message_index<len(message.content):
                if message.content[message_index] == '+':
                    bonus = int(message.content[message_index+1:])
                elif message.content[message_index] == '-':
                    bonus = -int(message.content[message_index+1:])
            
            dice_size = int(dice_size)
            dice_number = int(dice_number)

            diceResults = [await DiceClass.Roll(dice_size) for _ in range(0, dice_number)]
            if dice_size != 100:
                diceResultsSum = sum(diceResults)
            else:
                diceResultsSum = 0
                for obj in diceResults:
                    diceResultsSum += obj[0] + obj[1]
            await message.channel.send(f'{diceResults} [{diceResultsSum + bonus}] are thy {dice_number} throws...')
